...
//   secure: false, // ✅ Port 465 ke liye secure true
//   auth: {
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 30px; background-color: #ffffff;">
          ...
          <h2 style="color: #2D89EF; text-align: center;">Verify Your Identity 🔐</h2>
          ...
          <p style="font-size: 16px; color: #333;">Cheers,<br/>— The Team</p>
          <hr style="margin: 40px 0; border: none; border-top: 1px solid #e0e0e0;" />
          <p style="font-size: 12px; color: #999; text-align: center;">
            You’re receiving this email because your email was used to request an OTP.<br/>
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family: 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 30px; background-color: #ffffff;">
        ...
        <h2 style="color: #2D89EF; text-align: center;">Welcome Aboard! 🎉</h2>
        <p style="font-size: 16px; color: #333;">
          Hi <strong>${firstName || full_name || "there"}</strong>,
        </p>
        <p style="font-size: 16px; color: #333;">
        ...
        <hr style="margin: 40px 0; border: none; border-top: 1px solid #e0e0e0;" />
        <p style="font-size: 12px; color: #999; text-align: center;">
          You’re receiving this email because you signed up on our platform.<br/>
  ...
  try {
    ...
    if (
      ...
      return res.status(404).json({
        status: false,
        message:
          "You do not have access because your details do not match the required criteria.",
      });
    }
    ...
    where: {
    user_id: user.id,
    stripe_status: "active", // only active plans
    },
...
exports.clinicRegister = async (req, res) => {
  try {
    ...
    // ✅ Step 11: Send OTP to email instead of welcome mail
    try {
      const otp = generateOtp(); // You can use generateOtp() for random
      const otpExpiry = new Date(Date.now() + 10 * 60 * 1000);
      ...
      console.error("❌ OTP email error:", otpErr.message);
    }
...
const transporter = nodemailer.createTransport({
  ...
  tls: {
    ...
    const mailOptions = {
      from: `"Clinic Admin" <${process.env.MAIL_FROM}>`,
      to: email,
      subject: "Your Clinic Account Password Changed",
      html: `
        <h3>Hello,</h3>
        ...
        <br />
        <p>Please keep this password safe and do not share it with anyone.</p>
    ...
    return res.json({
      success: true,
      message: "Password updated and sent to clinic email successfully!",
    });
  } catch (error) {
