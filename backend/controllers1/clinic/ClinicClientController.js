...
const user_id = existingLinkedUser ? existingLinkedUser.id : null;
    ...
    if (id) {
      ...
      const enquiry = await SendEnquiry.findOne({
  where: {
    email: client.email, // check by email only
  },
});

if (enquiry) {
  await enquiry.update({ status: 1 });
}
             try {
...
Please login to the app and check.
...
exports.getEnquiries = async (req, res) => {
  try {
    ...
    return res.json({ success: true, data: formatted,limit:1,planmessage:"test Message" });
  } catch (err) {
