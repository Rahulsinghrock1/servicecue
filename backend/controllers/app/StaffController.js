...
const { getUserDetails, treatmentCalculation } = require('@helpers/commonHelper');
...
exports.AssignClient = async (req, res) => {
  try {
    ...
    const treatmentPlans = await Treatment.findAll({
      ...
      where: { status: 1 }, // ✅ only active treatments
...
//       },
//       attributes: ["id", "client_id","completed_at"],
//       raw: true,
//     });
...
exports.Clients = async (req, res) => {
  try {
    ...
    // Validate type (1 = active, 2 = inactive)
    if (![1, 2].includes(Number(type))) {
      return res.status(400).json({
        message: "Invalid type (must be 1 for active, 2 for inactive).",
        data: [],
    ...
    // Step 1: Fetch treatments created by this staff (active/inactive)
    const treatments = await Treatment.findAll({
      where: {
        created_by: loginUserId,
        status: Number(type) === 1 ? 1 : { [Op.ne]: 1 },
    ...
    // Map client_id -> array of treatment objects (with completed_at)
    const treatmentMap = new Map();
    treatments.forEach((t) => {
      if (!treatmentMap.has(t.client_id)) {
        treatmentMap.set(t.client_id, []);
      }
      treatmentMap.get(t.client_id).push(t);
    });

    const clientIds = [...treatmentMap.keys()];

    // Step 2: Get assigned clients for this staff
    const assignList = await AssignClient.findAll({
      where: {
        staff_id: loginUserId,
        client_id: { [Op.in]: clientIds },
    ...
    // Step 3: Build flattened response (one row per treatment)
    const finalList = [];
    ...
    for (const item of assignList) {
      ...
      const clientUserId = client.id;
      const clientTreatments = treatmentMap.get(clientUserId) || [];
      if (!clientTreatments.length) continue;

      for (const t of clientTreatments) {
        const treatmentId = t.id;

        const treatmentResult = await treatmentCalculation(treatmentId);

        console.log(treatmentResult);


        // ✅ Push each treatment as a separate record including completed_at
        finalList.push({
          id: item.id,
          staff_id: item.staff_id,
          client_id: item.client_id,
          clinic_id: item.clinic_id,
          createdAt: item.createdAt,
          completed_at: t.completed_at, // ✅ from Treatment table
          notes: item.notes,
          treatment_id: treatmentId,
          totaltreatments: treatmentResult.totaltreatments,
          totalproduct: treatmentResult.formattedPercentage,
          totalgoals: treatmentResult.totalgoals,
          client: {
            id: client.id,
            full_name: client.full_name,
            email: client.email,
...
exports.TreatmentPlan = async (req, res) => {
  try {
    ...
    if (user.user_role_id === 1) {
      // 🧍‍♂️ Case 1: Logged-in user is a client
      const clientUserId = user.id;

      const clients = await Client.findAll({
        where: { user_id: clientUserId }, // fetch client IDs linked to this user
        attributes: ['id'],
        raw: true
      });
      ...
      // ✅ Fetch all treatments for this client
      treatments = await Treatment.findAll({
        where: { client_id: clientIds },
        attributes: ["id", "clinic_id", "client_id", "completed_at"],
        raw: true,
      });

    } else {
      // 👨‍⚕️ Case 2: Other roles (staff/admin) → treatment_id is provided directly
      const treatment_id = req.body.client_id;

      if (!treatment_id) {
        return res.status(400).json({
          status: false,
          message: "treatment_id is required in request body."
        });
      }

      // ✅ Fetch only the treatment with given treatment_id
      treatments = await Treatment.findAll({
      ...
      if (!treatments || treatments.length === 0) {
        return res.status(404).json({
          status: false,
          message: "No treatment found with this ID.",
          data: [],
        });
      }
    }

    // ✅ Now `treatments` has the correct data for both cases
    ...
    const formattedPlans = plans.map((plan) => {
      ...
      if (plan.status === false) {
        // agar future start date hai
        if (today.isBefore(startDate)) {
          status = "Upcoming";
          treatmentDate = startDate;
        }
        // baaki sab active m aayega
        else {
          status = "Active";
        ...
        // sirf plan.status true hone par completed
        status = "Completed";
        treatmentDate = endDate || startDate;
      }
...
exports.clientTreatmentDetails = async (req, res) => {
  try {
    ...
    const treatmentPlanService = await TreatmentPlan.findAll({
      where: {
        treatment_id: id, // ✅ filter by treatment_id
        start_date: {
          [Op.gte]: today,
        },
      },
      attributes: ["id", "cat_id", "treatment_id", "start_date", "end_date", "price"],
      raw: true,
    });
    ...
    const treatmentResult = await treatmentCalculation(id);
...
exports.clientTreatmentgoal = async (req, res) => {
  ...
  try {
    ...
    const responseData = {
      ...
      treatment_statusss: treatment.status,
      treatment_status: treatment.status === true ? false : true
  ...
  try {
    ...
    const message = {
      ...
      notification: {
        ...
        body: `${loginUser.full_name} treatment ${goal.name} is completed. to notify staff for review and follow-up.`,
      },
      data: {
        type: "Goal Completed",
    ...
    await Notifications.create({
      ...
      user_id: treatmentCreator.id,
      route_id: goal.treatment_id,
      type: "Goal Completed",
    });
      ...
      if (treatmentCreator && treatmentCreator.push_token) {
        const message = {
          ...
          notification: {
            ...
            body: `Your treatment has been completed. The follow-up was handled by ${staffUser.full_name}.`
          },
          data: {
...
//     const treatment = await TreatmentPlan.findOne({ where: { treatment_id: treatment_id } });
//     if (!treatment) {
//       return res.status(404).json({
//         status: false,
//         message: "Treatment not found.",
...
const timezone = user?.timezone || "Australia/Sydney";  // default
// --- Today according to user's timezone ---
const userTodayString = new Date().toLocaleString("en-US", {
  timeZone: timezone
});
const userToday = new Date(userTodayString);
userToday.setHours(0, 0, 0, 0);
// --- Normalize parsed date ---
parsedDate.setHours(0, 0, 0, 0);
// --- Future date check based on user timezone ---
if (parsedDate > userToday) {
  return res.status(400).json({
    status: false,
    message: "Cannot mark product as done for a future date.",
  });
...
exports.productrepurchase = async (req, res) => {
  try {
    ...
    // ✅ Create a new repurchase record
    ...
    // ✅ Find the matching ProductPrescription record
    const prescription = await ProductPrescriptions.findOne({
      where: { product_id, treatment_id },
      attributes: ["id", "expected_end_time", "expected_days"],
    });

    if (prescription) {
      // ✅ Parse expected_days to integer
      const extraDays = parseInt(prescription.expected_days, 10) || 0;

      let expectedEndDate = prescription.expected_end_time
        ? new Date(prescription.expected_end_time)
        : new Date(); // fallback to today if null

      // ✅ Safely add expected_days
      if (extraDays > 0 && !isNaN(extraDays)) {
        expectedEndDate.setDate(expectedEndDate.getDate() + extraDays);

        // ✅ Update expected_end_time in DB
        await prescription.update({
          expected_end_time: expectedEndDate,
        });

        console.log(`✅ Extended expected_end_time by ${extraDays} days for product_id=${product_id}`);
      } else {
        console.warn(`⚠️ expected_days not found or invalid for product_id=${product_id}`);
      }
    } else {
      console.warn(`⚠️ No ProductPrescription found for treatment_id=${treatment_id}, product_id=${product_id}`);
    }

    return res.status(200).json({
      status: true,
      message: "Product repurchase successful and expected_end_time updated dynamically.",
    });

  } catch (err) {
    console.error("❌ productrepurchase Error:", err);
    return res.status(500).json({
      status: false,
      message: "Internal server error.",
      error: process.env.NODE_ENV !== "production" ? err.message : undefined,
...
exports.newdashboard = async (req, res) => {
  ...
  try {
    ...
    const updatedClinicList = await Promise.all(
      cliniclist.map(async (clinic) => {
        // ✅ Check if user follows this clinic
        const isFollowed = await ClinicFollow.findOne({
          where: { user_id, clinic_id: clinic.id },
          attributes: ["id"],
          raw: true,
        });

        const favorite_status = !!isFollowed; // true if found

        // ✅ Handle reviews & rating
        const reviews = clinic.reviews || [];
        const totalRating = reviews.reduce((sum, r) => sum + r.rating, 0);
        const avgRating = reviews.length ? (totalRating / reviews.length).toFixed(1) : 0;

        // ✅ Return formatted clinic data
        return {
          id: clinic.id,
          ...
          avatar: clinic.avatar
            ...
            : baseUrl + "uploads/clinics/no-image.jpg",
          status: clinic.status,
          address: clinic.address,
          rating: avgRating,
          distance: "2.5",
          totalReviews: reviews.length,
          favorite_status, // ✅ true or false dynamically from DB
        };
      })
    );
  ...
  // NEW STATUS LOGIC
  if (tp.status === true) {
    // ✔️ Completed only when tp.status = true
    status = "completed";
  } else {
    // ✔️ tp.status = false → Upcoming / Active logic
  ...
  return {
    ...
    const formattedPercentage = !isNaN(percentage) && isFinite(percentage) ? percentage.toFixed(2) : 0;
...
products.forEach(p => {
  productDataMap[p.id] = {
    ...
    // Group goals by treatment
    const treatmentGoalsMap = {};
    allGoals.forEach(goal => {
      if (!treatmentGoalsMap[goal.treatment_id]) {
        treatmentGoalsMap[goal.treatment_id] = [];
      }
      treatmentGoalsMap[goal.treatment_id].push(goal);
    });

    const Goalsreminders = [];

    // Iterate over each treatment's goals
    for (const treatmentId of Object.keys(treatmentGoalsMap)) {
      const goals = treatmentGoalsMap[treatmentId];

      // Check if at least one goal is complete in this treatment
      const hasCompletedGoal = goals.some(
        g => g.status === 1 || g.status === "1" || g.status === "completed"
      );

      if (hasCompletedGoal) {
        // Only show incomplete goals if at least one goal is complete
        ...
        incompleteGoals.forEach(g => {
          ...
          Goalsreminders.push({
            ...
            treatmentImages: treatmentImages.length ? treatmentImages : [{ key: "default", image_url: DEFAULT_IMAGE }],
          });
        });
      }
    }
    ...
    const productClinicMap = treatmentProducts.map(p => {
      ...
      return {
        ...
        clinic_address: clinicMap[treatment?.clinic_id]?.address || null,
      };
    });
    ...
    const productRepurchase = productss
      .map(p => {
        ...
        if (prescription?.start_time && prescription?.expected_end_time) {
          const today = new Date();
          const expectedEnd = new Date(prescription.expected_end_time);

          // ✅ Calculate days remaining until expected_end_time
          diffDays = Math.ceil((expectedEnd - today) / (1000 * 60 * 60 * 24));
          ...
          // ✅ Trigger reminder 3 days before expected_end_time
          showReminder = diffDays <= 3;
        }
        ...
        return {
          ...
          expected_end_time: prescription?.expected_end_time || null, // ✅ added
...
exports.TreatmentDetails = async (req, res) => {
  ...
  title: cat.title,
  image: cat.image
    ? `${process.env.BASE_PATH || "http://localhost:5000"}/${cat.image}`
    : null,
}));
    }
  ...
  try {
    ...
    const cleanPath = (path) => {
      ...
      return path
        ...
        .replace(/:\/\/[^/]+/, "");   // remove http://domain.com
    };

    // ---------------------------------------------------------
    // NEW UPLOADED IMAGES (if any)
    // ---------------------------------------------------------
    const frontUpload = req.files?.front
      ? `/uploads/clinic_images/${req.files.front[0].filename}`
      : null;

    const leftUpload = req.files?.left
      ? `/uploads/clinic_images/${req.files.left[0].filename}`
      : null;

    const rightUpload = req.files?.right
      ? `/uploads/clinic_images/${req.files.right[0].filename}`
      : null;
    ...
    if (typeof req.body?.treatments === "string") {
      try {
        treatments = JSON.parse(req.body.treatments);
      } catch { }
    }
    ...
    treatments = treatments.filter(
      (t) => t.cat_id && t.start_date && t.end_date
    );

    if (!treatments.length) {
      return res.status(400).json({
        status: false,
        message: "At least one treatment plan is required",
      });
    }

    // ---------------------------------------------------------
    // CREATE OR UPDATE MAIN TREATMENT
    // ---------------------------------------------------------
    let mainTreatment;
    ...
    if (edit_id) {
      ...
      // FINAL IMAGES (using upload OR cleaned old images)
      const finalFront = frontUpload || cleanPath(mainTreatment.front);
      const finalLeft = leftUpload || cleanPath(mainTreatment.left);
      const finalRight = rightUpload || cleanPath(mainTreatment.right);
      ...
      // Send notification
      const creator = await User.findOne({ where: { id: mainTreatment.client_id } });
      const staffUser = await User.findOne({ where: { id: mainTreatment.created_by } });

      if (creator?.push_token) {
        const message = {
          token: creator.push_token,
          notification: {
            title: "Treatment Update",
            body: `your Treatment has been updated. Kindly review the details.`,
          },
          data: {
            type: "Treatment Update",
            id: edit_id.toString(),
    ...
    // ---------------------------------------------------------
    // TREATMENT PLANS (Delete + Create)
    // ---------------------------------------------------------
    if (edit_id) {
      const today = moment().format("YYYY-MM-DD");
      await TreatmentPlan.destroy({
        where: { treatment_id: edit_id, start_date: { [Op.gte]: today } },
      });
    }

    const finalFrontImg = frontUpload || cleanPath(mainTreatment.front);
    const finalLeftImg = leftUpload || cleanPath(mainTreatment.left);
    const finalRightImg = rightUpload || cleanPath(mainTreatment.right);
    ...
    if (productsArray.length) {
      ...
      // ---------------------------------------------------------
      // LINK LAST PRESCRIPTION: UPDATE treatment_id for new ones
      // ---------------------------------------------------------
      await ProductPrescriptions.update(
        { treatment_id: mainTreatment.id },
        {
          where: {
            staff_id: loginUserId,
            client_id: client_id,
            treatment_id: null,
            product_id: { [Op.in]: productsArray }
          }
        }
      );

      // ---------------------------------------------------------
      // SOFT DELETE PRESCRIPTIONS NOT IN NEW LIST
      // ---------------------------------------------------------
      await ProductPrescriptions.destroy({
        where: {
          staff_id: loginUserId,
          client_id: client_id,
          treatment_id: mainTreatment.id,
          product_id: { [Op.notIn]: productsArray }
        }
      });
    ...
    return res.status(201).json({
      ...
      treatment_id: mainTreatment.id,
      plans_created: planResults.length,
    });
  } catch (err) {
    console.log(err);
    return res.status(500).json({
