...
function getTimezoneFromLatLon(lat, lon) {
  try {
    const tz = geoTz.find(lat, lon);
    return tz && tz.length ? tz[0] : null;
  } catch (err) {
    return null;
  }
}
...
exports.Clinic = async (req, res) => {
  try {
    ...
    let filteredClinics = clinics
      ...
      if (lat && long) {
  const timezone = getTimezoneFromLatLon(lat, long);
...
exports.addReview = async (req, res) => {
  try {
    ...
    if (!rating || !clinic_id) {
      return res.status(400).json({ message: "Rating and clinicId are required", status: false });
    }

    const userId = req.user.id;

    // ✅ Check if the user already reviewed this clinic
    const existingReview = await ClinicReview.findOne({
      where: { clinic_id, user_id: userId },
    });
