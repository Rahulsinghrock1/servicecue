...
const ContactUsEnquiry = require('@models/ContactUsEnquiry');
...
if (!name || name.trim() === '') {
  ...
  tls: {
    rejectUnauthorized: false,
      ...
      await transporter.sendMail({
        from: `"ServiceCue" <${process.env.MAIL_FROM}>`,
        to: clinic.email,
 ...
 <div style="font-family: 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 30px; background-color: #ffffff;">
  ...
  <p style="font-size: 16px; color: #333;">Dear <strong>${clinic.full_name || "there"}</strong>,</p>
...
exports.clinicProfessionals = async (req, res) => {
  try {
    ...
    const specialistsData = await User.findAll({
      where: {
        user_role_id: 2,
        created_by: clinic_id
      },
      attributes: ["id", "full_name", "avatar", "experience", "designation"],
      raw: false  // important: false so that image/virtuals come from model
    });
    ...
    // Convert and add specialists key
    const specialists = specialistsData.map(user => {
      const plainUser = user.get({ plain: true });
      return {
        ...plainUser,
        specialists: plainUser.designation, // set designation value in specialists key
      };
    });
...
//     // 🔹 Total Enquiries
//     const totalEnquiries = await SendEnquiry.count({
//       where: {
//         clinic_id: clinic_id,
//       },
//     });
...
console.log("totalProductss",treatmentIds);
  ...
  include: [
    {
      ...
      required: true, // ensures only existing staff are counted
      // if paranoid model:
...
exports.support = async (req, res) => {
  try {
const data = [
  { email: "admin@servicecue.com.au",aboutApp: "https://servicecue.com/about",termsConditions: "https://servicecue.com.au/terms-and-conditions",privacyPolicies: "https://servicecue.com.au/privacy-policy",fAndQs: "https://servicecue.com/faqs"},
];
    return res.status(200).json({
      message: 'List fetched successfully!',
      data: data,
...
exports.sendNewsletter = async (req, res) => {
  try {
    ...
    // ✅ Create Enquiry
    const enquiry = await Newsletter.create({ email });
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 30px; background-color: #ffffff;">
            ...
            <h2 style="color: #2D89EF; text-align: center;">You've Got a Newsletter 📩</h2>
...
exports.sendContactUsEnquiry = async (req, res) => {
  try {
    const { email, name, subject, message } = req.body; 

    // ✅ Create Enquiry
    const enquiry = await ContactUsEnquiry.create({ email, name, subject, message });
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 30px; background-color: #ffffff;">
            ...
            <h2 style="color: #2D89EF; text-align: center;">You've Got a Contact Us Enquiry 📩</h2>
            ...
            <div style="background-color: #f4f8fb; border-left: 4px solid #2D89EF; padding: 15px; margin: 20px 0;">             
              ...
              <p style="font-size: 16px; color: #333;"><strong>Message:</strong> ${message}</p>
            </div>
    ...
    console.error('Send ContactUsEnquiry Error:', error);
    return res.status(500).json({
      status: false,
      message: 'Something went wrong!',
      error: error.message,
...
exports.deleteNotifications = async (req, res) => {
  try {
    ...
    const { id } = req.body;  // <-- check if notification id passed

    // If ID is sent → delete that specific notification
    if (id) {
    ...
    // If ID not sent → delete logged-in user's all notifications
    const deletedAll = await Notifications.destroy({
      where: { user_id }
    });
...
exports.sendTreatmentNotifications = async (req, res) => {

  console.log("Crone Is Working Fine");
  try {
    const today = new Date();
    const tomorrow = new Date();
    tomorrow.setDate(today.getDate() + 1);

    // Fetch all treatment plans starting today or tomorrow
    ...
    if (!treatmentPlans || treatmentPlans.length === 0) {
      return res.status(200).json({
        status: true,
        message: "No treatments scheduled for today or tomorrow."
      });
    }

    for (let plan of treatmentPlans) {
      if (!plan.treatment_id || !plan.client_id) {
       // console.warn(`Skipping plan with missing treatment_id or client_id: ${JSON.stringify(plan)}`);
        continue; // skip invalid plans
      }

      // Fetch treatment info
      const treatment = await Treatment.findOne({
        where: { id: plan.treatment_id },
        attributes: ["id", "created_by"],
        raw: true
      });

      if (!treatment || !treatment.created_by) {
        //console.warn(`Skipping plan due to missing treatment or created_by: ${plan.treatment_id}`);
        continue;
      }
      ...
      // Staff notification on start date
      if (staffUser?.push_token?.trim() && new Date(plan.start_date).toDateString() === today.toDateString()) {
        await sendPushNotification({
          token: staffUser.push_token,
          title: "Treatment Started 🏥",
          body: `Treatment "${plan.name}" has started today. Please follow up with the client.`
        });
      }

      // Fetch client user_id from Client table
      const client = await Client.findOne({
        where: { id: plan.client_id },
        attributes: ["user_id"],
        raw: true
      });
      ...
      if (clientUser?.push_token?.trim()) {
        ...
        if (today.toDateString() === oneDayBefore.toDateString()) {
          await sendPushNotification({
            token: clientUser.push_token,
            title: "Upcoming Treatment ⏰",
            body: `Your treatment "${plan.name}" is scheduled for tomorrow.`
          });
        }

        // On treatment start date
        if (today.toDateString() === startDate.toDateString()) {
          await sendPushNotification({
            token: clientUser.push_token,
            title: "Treatment Today 🏥",
            body: `Your treatment "${plan.name}" starts today.`
          });
        }
      }
    }
    ...
    return res.status(200).json({
      status: true,
      message: "Notifications processed for treatments."
    });
