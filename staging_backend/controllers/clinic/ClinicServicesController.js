...
const activeSubscription = await Subscription.findOne({
  ...
  order: [["id", "DESC"]]
    ...
    const STAFF_LIMITS = {
      1: 50,        // Plan ID 1 → 1 staff
      2: 100,        // Plan ID 2 → 5 staff
      3: Infinity,
    ...
    const totalStaff = await ClinicServices.count({
      where: {
        clinic_id: clinicId,
      },
    });
    ...
    if (staffLimit !== Infinity && totalStaff >= staffLimit) {
      return res.status(400).json({
        status: false,
        message: `You’ve reached your services  limit for the ${plan.title} plan (${staffLimit}). Please upgrade to add more services .`,
      });
    }
