...
const handleOpenAssignedClients = async (staff) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/assignedstafflist`, {
      client_id: staff,
    });
...
const handleChangeStatus = async () => {
  ...
  try {
    const response = await axios.patch(
      `${API_BASE_URL}/client/${selectedRow.id}/toggle-status`
    );
...
const handleConfirmDelete = async () => {
  ...
  try {
    ...
    const res = await axios.delete(`${API_BASE_URL}/client/delete/${selectedRow.id}`);
...
const handleConfirmRemove = async () => {
  ...
  try {
    ...
    const response = await axios.post(`${API_BASE_URL}/unassignStaff`, {
...
console.log(selectedRow);
    toast.success(response.data.message);
    handleOpenAssignedClients(selectedRow.client_id);
    setSelectedStaff(null);
    setSelectedRow(null);
    fetchStaffs();
  } catch (error) {
...
useEffect(() => {
  ...
  let age = today.getFullYear() - birthDate.getFullYear(); // Calculate the age in years
  ...
  return `${age} years`; // Return the age as a string
};
        ...
        <div className="flex-grow-1">
          <h4 className="fw-bold mb-0">
            ...
            <span className="badge badge-soft-primary fs-13 fw-medium ms-2">
              Total Clients : {sites.length}
            </span>
          </h4>
        </div>
        <div className="text-end d-flex">
          <Link
            href="/clinic/client/create"
            className="btn btn-primary ms-2 fs-13 btn-md"
          >
            <FaPlus className="me-1" /> New Client
          </Link>
        </div>
      </div>
          ...
          <tbody>
            {loading ? (
              ...
              paginatedSites.map((row) => (
                <tr key={row.id}>
                  <td>
                  <Link href={`/clinic/client/show/${row.id}`} className="text-decoration-none text-dark">
  <div className="d-flex align-items-center gap-2 cursor-pointer">
    ...
    <div>
      ...
      <div className="text-muted small">
        {row.dob ? calculateAge(row.dob) : "N/A"},{row.gender || "N/A"}
      </div>
    </div>
  </div>
</Link>
...
<div className="d-flex justify-content-between align-items-center">
                  ...
                  <td>
                    <span
                    className={`${
                    row.status ? "badge badge-soft-success rounded text-success border border-success fs-13 fw-medium" : "badge gray-ms-border rounded gray-ms border fs-13 fw-medium"
                    }`}
                    >
                      ...
                      {dropdownOpen === row.id && (
                        ...
                        >
                          ...
                          <li>
                            <Link
                              className="dropdown-item"
                              href={`/clinic/client/edit/${row.id}`}
                            >
                              Edit
                            </Link>
                          </li>
...
<div className="modal fade" id="edit_status">
  <div className="modal-dialog modal-dialog-centered modal-sm">
    <div className="modal-content">
      ...
      >
       ...
       <div className="border-bottom d-flex align-items-center justify-content-between pb-3 px-3">
          ...
          <h5 className="offcanvas-title fs-18 fw-bold">Assigned Staff  <span className="badge badge-soft-primary border pt-1 px-2 border-primary fw-medium ms-2">{assignedClients.length} Staff</span></h5>
          <button
            type="button"
            className="btn-close"
    ...
    <h6 class="bg-light py-0 px-3 text-dark fw-bold"> Assigned Staff </h6>
  ...
  <div key={index} className="d-flex align-items-center justify-content-between mb-4">
    <div className="d-flex align-items-center">
      <a href="javascript:void(0);" className="avatar me-2 flex-shrink-0">
        <img src={item.client.avatar} alt={item.client.full_name} className="rounded-circle" />
      </a>
      <div>
...
>
  ...
  <div className="modal-dialog ">
    <div className="modal-content">
      <div className="modal-header">
        <h4 className="text-dark modal-title fw-bold">Assign Staffs</h4>
        <button
          type="button"
      ...
      <form onSubmit={handleAssignClients}>
        <div className="modal-body">
          <label className="mb-2 fw-semibold">Select Staffs</label>
  <Select
  isMulti
  options={[{ label: "Select All Staffs", value: "all" }, ...clients]}
  ...
  onChange={(selected) => {
    ...
    // Check if "Select All Staffs" is chosen
    if (selected.some((opt) => opt.value === "all")) {
      if (formData.clients.length === clients.length) {
        // Agar already all selected hai → clear
        setFormData({ ...formData, clients: [] });
      } else {
        // Saare clients select kar do
